...
public interface PedidoRepository extends JpaRepository<Pedido, Long> {
	...
	@Query("SELECT p FROM tb_pedido p LEFT JOIN tb_comanda c ON p.codigoComanda = c.id WHERE c.codigoEstabelecimento = ?1 AND p.codigoStatus NOT IN (0, 3, 6) ORDER BY p.dataCadastro DESC")
